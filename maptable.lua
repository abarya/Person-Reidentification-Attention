...
function MapTable:_extend(n)
   ...
   --assert(n==3,"provide 3 inputs")
   p,g=self.module:getParameters()
   print("size of original",p:size(1))
   for num = 2, n do
      if not self.modules[i] then
         if self.shared then
           self.modules[num] = self.module:clone()
           for i=1,#self.module.forwardnodes do
             if(self.module.forwardnodes[i].data.module~=nil and self.module.forwardnodes[i].data.module.weight~=nil) then
                self.module.forwardnodes[i].data.module:share(self.modules[num].forwardnodes[i].data.module, 'weight','bias','gradWeight','gradBias');
                -- print(self.module.forwardnodes[i].data.annotations.name)
             end 
             if(self.module.forwardnodes[i].data.annotations.name=='attention_module') then
              atten1=self.module.forwardnodes[i].data.module.lstm
              atten2=self.modules[num].forwardnodes[i].data.module.lstm
              for j=1,#atten1.forwardnodes do
                if(atten1.forwardnodes[j].data.module~=nil and atten1.forwardnodes[j].data.module.weight~=nil) then
                  ...
                  -- print(atten1.forwardnodes[j].data.annotations.name)
                end
              end 
             end  
            end 
      ...
      p,g=self.modules[num]:getParameters()
      print(p:size(1),num)
   end
end
